...
// Same as above, but also returns the raw key bytes for KEK storage
async function deriveKeyFromPasswordRaw(password, saltBytes, iterations = 200_000) {
...
document.getElementById("unlock").addEventListener("click", async () => {
  ...
  chrome.runtime.sendMessage({ type: "UNLOCK_ALMA_KEY", password }, (res) => {
...
if (haveK && haveS && haveI) {
  ...
  const base  = await crypto.subtle.importKey("raw", enc.encode(password), { name:"PBKDF2" }, false, ["deriveKey"]);
  const key   = await crypto.subtle.deriveKey({ name:"PBKDF2", salt, iterations: iters, hash:"SHA-256" }, base, { name:"AES-GCM", length:256 }, true, ["encrypt","decrypt"]);
  const raw   = new Uint8Array(await crypto.subtle.exportKey("raw", key));
...
document.getElementById("save")?.addEventListener("click", async () => {
  ...
  if (!apiKey) {
    await store.set({ ALMA_REGION: region });
  ...
  if (!pw) { setStatus("status","Enter password in Admin unlock to encrypt.","err"); return; }
  ...
  try {
    ...
    if (probe !== meta.ALMA_KEK_B64) {
      setStatus("status","Wrong password for encryption.","err"); return;
    }
...
document.getElementById("toggleKey")?.addEventListener("click", () => {
  ...
  if (!__apiKeyRevealed) {
    chrome.runtime.sendMessage({ type: "REVEAL_ALMA_KEY" }, (res) => {
      if (chrome.runtime.lastError) {
...
document.getElementById("clear").addEventListener("click", async () => {
  ...
  await store.remove(["ALMA_API_KEY","ALMA_API_KEY_C","ALMA_API_KEY_IV","ALMA_API_KEY_SALT","ALMA_KDF_ITERS", "ALMA_PW_SALT","ALMA_KEK_B64"]);
...
document.getElementById("test").addEventListener("click", () => {
  ...
  chrome.runtime.sendMessage({ type: "TEST_ALMA_CONNECTIVITY" }, (resp) => {
    if (chrome.runtime.lastError) {
